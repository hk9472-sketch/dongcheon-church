...
type ViewMode = "group" | "division" | "date";
...
function downloadExcel(filename: string) {
  ...
  if (!table) return;
  const html = `<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="utf-8"><style>td,th{border:1px solid #999;padding:2px 5px;text-align:center;font-size:11px;} th{background:#f0f0f0;font-weight:bold;}</style></head><body>${table.outerHTML}</body></html>`;
...
export default function ReportPage() {
  ...
  // 합계 계산
  function sumGroup(rows: GroupRow[]) {
    const t: Record<string, number> = {};
    for (const c of groupCols) t[c.key] = 0;
    for (const r of rows) for (const c of groupCols) t[c.key] += (r as unknown as Record<string, number>)[c.key] || 0;
    return t;
  }
  function sumDiv(rows: DivisionRow[]) {
    const t: Record<string, number> = {};
    for (const c of divCols) t[c.key] = 0;
    for (const r of rows) for (const c of divCols) t[c.key] += (r as unknown as Record<string, number>)[c.key] || 0;
    return t;
  }

  const tabs: { key: ViewMode; label: string }[] = [
    { key: "group", label: "구역별" },
    { key: "division", label: "반사별" },
    { key: "date", label: "날짜별" },
  ];

  const hasData = viewMode === "group" ? groupData.length > 0
    : viewMode === "division" ? divisionData.length > 0
    : dateData.length > 0;

  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      <div className="px-4 py-3 border-b border-gray-200">
        <h1 className="text-base font-bold text-gray-800 mb-3 flex items-center gap-2">보고서 조회 <HelpButton slug="council-report" /></h1>
        <div className="flex flex-wrap gap-3 items-end">
          ...
          {hasData && (
            <button onClick={() => downloadExcel(`보고서조회_${viewMode}_${fromDate}_${toDate}.xls`)}
      ...
      <div className="overflow-x-auto p-4">
        {!searched ? (
          ...
          <table className="w-full text-xs border-collapse print:text-[10px]">
            <thead>
              <tr className="bg-gray-100">
                <th rowSpan={2} className="border border-gray-300 px-2 py-1 text-center w-20">날짜</th>
                <th colSpan={4} className="border border-gray-300 px-1 py-1 text-center bg-blue-50">장년반</th>
                <th colSpan={4} className="border border-gray-300 px-1 py-1 text-center bg-green-50">중간반</th>
                <th colSpan={2} className="border border-gray-300 px-1 py-1 text-center bg-purple-50">성경</th>
                <th rowSpan={2} className="border border-gray-300 px-1 py-1 text-center bg-yellow-50 w-10">기도</th>
            ...
            <tbody>
              {dateData.map((row) => {
                ...
                return (
                  <tr key={row.date} className="hover:bg-gray-50">
                    <td className="border border-gray-300 px-2 py-1 text-center font-medium">{formatDateShort(row.date)}</td>
                    {groupCols.map((c) => (
                      <td key={c.key} className="border border-gray-300 px-1 py-1 text-center">
                        {(row as unknown as Record<string, number>)[c.key] || ""}
                      </td>
              ...
              {(() => {
                const t: Record<string, number> = {};
                for (const c of groupCols) t[c.key] = 0;
                for (const r of dateData) for (const c of groupCols) t[c.key] += (r as unknown as Record<string, number>)[c.key] || 0;
                const total = groupCols.reduce((sum, c) => sum + (t[c.key] || 0), 0);
                return (
                  <tr className="bg-yellow-50 font-bold">
                    <td className="border border-gray-300 px-2 py-1 text-center">합계 ({dateData.length}일)</td>
                    {groupCols.map((c) => (
                      <td key={c.key} className="border border-gray-300 px-1 py-1 text-center">{t[c.key] || ""}</td>
                    ))}
                    <td className="border border-gray-300 px-1 py-1 text-center bg-orange-100/50">{total || ""}</td>
