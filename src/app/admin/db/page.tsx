...
function UserMigrationTab() {
  ...
  const handleTruncateUser = async () => {
    if (!confirm("[User] 테이블의 모든 데이터를 삭제하고 AUTO_INCREMENT를 초기화하시겠습니까?\n\n로그인 세션(Session)도 함께 초기화됩니다.\n이 작업은 되돌릴 수 없습니다.")) return;
    ...
    try {
      ...
      const data = await res.json();
      setTruncateMsg(data.success ? "User 테이블 초기화 완료" : `오류: ${data.error}`);
      setUsers([]);
  ...
  const executeDirectMigration = async () => {
    const toMigrate = users.filter((u) => !u.alreadyExists).length;
    if (!confirm(`${toMigrate}명의 사용자를 이관하시겠습니까?\n\n기존에 존재하는 사용자는 건너뜁니다.`)) return;
    setMigrating(true);
    setError("");
    setResult(null);
    try {
  ...
  const newUsers = users.filter((u) => !u.alreadyExists);
  const existingUsers = users.filter((u) => u.alreadyExists);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-red-200 overflow-hidden">
        <div className="px-4 py-3 bg-red-50 border-b border-red-200">
          <h2 className="text-sm font-bold text-red-700">이관 전 테이블 초기화</h2>
          <p className="text-xs text-red-600 mt-1">이관 전에 기존 사용자 데이터를 삭제하고 AUTO_INCREMENT를 초기화합니다.</p>
        </div>
      ...
      {result && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <div className="text-sm text-green-700 space-y-1">
            <p>전체: {result.total}명 | 이관됨: <strong>{result.migrated}명</strong> | 건너뜀: {result.skipped}명 | 오류: {result.errorCount}건</p>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-gray-50">
                {users.map((u) => (
                  <tr key={u.no} className={`hover:bg-gray-50 ${u.alreadyExists ? "opacity-50" : ""}`}>
