...
# .env 에 추가 (예: 값을 그대로 붙여넣기)
# RRN_ENCRYPTION_KEY="a3f1c2...(64자)"
...
**분실 시 복구 불가**. 키 백업:
- 패스워드 매니저 (1Password/Bitwarden 등)
- 오프라인 보관 (종이, 금고)
- **절대 git 커밋 금지** — `.gitignore` 의 `.env` 규칙 유지 확인

키 로테이션: 저장 문자열이 `v1:` prefix 로 시작하므로 v2 도입 시 호환 유지. 기존 데이터는 관리 스크립트로 일괄 재암호화.
