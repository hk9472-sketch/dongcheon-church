...
<h3>9. 글 등록 옵션 (본문 아래 체크박스 · 라디오)</h3>
<ul>
  <li><strong>☑ HTML</strong> — 켜면 위 툴바가 제공하는 <em>서식이 있는 본문</em>으로 저장되고, 끄면 입력한 글자 그대로 <em>평문</em>으로 저장됩니다. 서식·이미지·표를 쓰려면 켜 두세요. (게시판에 따라 기본값이 다를 수 있습니다.)</li>
  <li><strong>☑ 비밀글</strong> (로그인 시에만 표시) — 체크하면 <em>작성자 본인과 관리자만</em> 내용을 볼 수 있는 글이 됩니다. 상담·요청 등 민감한 글에 사용하세요. 목록에는 제목만 보이고 본문은 가려집니다.</li>
  <li><strong>☑ 공지사항</strong> — 체크하면 해당 글이 <em>목록 상단에 고정</em>되고 "공지" 뱃지가 붙습니다. 번호·페이지네이션에도 포함되지 않아 최신 글에 밀리지 않습니다. (게시판 권한이 있는 경우에만 체크 가능)</li>
  <li><strong>◉ 댓글허용</strong> — 이 글의 댓글 정책을 결정합니다. 등록 후에도 수정 시 변경할 수 있습니다.
    <ul>
      <li><strong>수정추가</strong> — 댓글을 <em>작성·수정·삭제 모두</em> 허용. 비회원 댓글은 이 옵션에서만 수정·삭제할 수 있습니다 (비밀번호 확인).</li>
      <li><strong>추가</strong> — 댓글 작성은 허용하지만 <em>한 번 작성한 뒤에는 수정·삭제할 수 없습니다</em>. 응답 기록을 보존해야 하는 공지/공고·설문에 적합합니다.</li>
      <li><strong>미사용</strong> — 이 글에는 <em>댓글을 아예 달 수 없습니다</em>. 일방 공지에 사용하세요.</li>
    </ul>
  </li>
</ul>
